    ...
    public function importexercises(){
        ...
        if(isset($post['current_group_id'])){
            ...
            foreach($post['exercise_id'] as $importExerciseId){
                ...
                $testGroup = $groups->getByImportId($importExercise[0]->group_id);

                // if this group already imported - we use that id (db id, not imported) in imported exercise
                if(count($testGroup) > 0 &&
                   !(isset($post['noimport_id']) 
                        && in_array($importExercise[0]->group_id, $post['noimport_id']))){
                    ...
                    if(isset($post['noimport_id']) && in_array($importExercise[0]->group_id, $post['noimport_id'])){
                        $groupId = $post['current_group_id'];
                    }else{
                ...
                $result = $exercises->addItem(array(
